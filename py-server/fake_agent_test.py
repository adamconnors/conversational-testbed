...
class TestFakeAgent(unittest.TestCase):
    def test_chat(self):
        ...
        message_history = test_utils.build_message_history_for_test(["Hello", response, "How are you?"])
        response, world_state = agent.chat(AgentState("How are you?", message_history, world_state))        
        self.assertEqual(response, "Fake response to message 3: How are you?")
        self.assertEqual(world_state, {"last_message": "How are you?", "message_count": 3})
